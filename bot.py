...
def main():
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            logger.error(f"🔴 КОНФЛИКТ: Другой бот уже запущен (попытка {retry_count}/{MAX_RETRIES})")
            logger.info(f"⏳ Ожидание {wait_time} секунд перед перезапуском...")
            
            if retry_count < MAX_RETRIES:
                time.sleep(wait_time)
            else:
                logger.error("🚨 Превышено максимальное количество попыток. Бот остановлен.")
                break
            ...
            logger.error(f"💥 Ошибка: {e} (попытка {retry_count}/{MAX_RETRIES})")
